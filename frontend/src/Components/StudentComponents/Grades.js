...
class Grades extends Component {
    ...
    componentDidMount(){
        Axios.get("http://localhost:5000/api/grades/get_subject_grades?student_id="+this.props.user_id+"&subject_id="+this.props.subject.subject_id)
        .then(response => {
            this.setState({
    ...
    handleSubmit = e => {
        ...
        if(this.props.currentUser.user.user_id === this.props.subject.professor_id){
            ...
            Axios.post("http://localhost:5000/api/grades/set_grade", data)
            .then(response => {
                this.setState({
                    quarters: response.data.subject_grades,
                    dataIsLoaded: true,
